...
def swap_and_inc(val):
    # Two's complement inversion: invert all bits and add 1
    inverted = (~val) & 0xFFFF  # Invert all 16 bits
    return (inverted + 1) & 0xFFFF  # Add 1 and mask to 16 bits
...
def main():
        ...
        while True:
            try:
                ...
                if real_val < 0:
                    ...
                    print(
                        f"\t{i + 1}) Hex format of {raw_input}: 0x{transformed_val:04X}"
                    )
                    raw_ints.append(-fixed_point_val)
                else:
                    print(
                        f"\t{i + 1}) Hex format of {raw_input}: 0x{fixed_point_val:04X}"
                    )
                    raw_ints.append(fixed_point_val)
                break
            except ValueError:
    ...
    print(
        f"\t6) Result after differencing two real numbers: {round(final_diff_real, 2)}"
    )
